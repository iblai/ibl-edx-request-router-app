...
def manager_api_request(
    ...
    if MANAGER_AUTH_ENABLED:
        headers["Authorization"] = "Bearer {}".format(
            get_app_access_token(MANAGER_AUTH_APP_ID)
        )
...
def convert_manager_proxy_params(params):
    ...
    if "username" in params:
        try:
            ...
            log.error(
                "Error converting username to user_id (User does not exist): %s",
                username,
            )
        except Exception:
...
def manager_proxy_request(request, endpoint_path=""):
    ...
    request_kwargs = {"params": convert_manager_proxy_params(request.query_params), "timeout": MANAGER_PROXY_TIMEOUT,
                      "headers": headers, "verify": MANAGER_VERIFY_SSL,
                      "json": convert_manager_proxy_params(request.data)}

    log.info("Manager proxy request: %s %s", request.method, url)
    return requests.request(request.method, url, **request_kwargs)
